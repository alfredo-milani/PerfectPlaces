...
public class ControlloreDisponibilitàAlbergo extends ControlloreDisponibilità {
    ...
    public boolean controllo(Locazione loc, GregorianCalendar gcInizio, GregorianCalendar gcFine, String numeroPersone) throws DeserializzazioneException, SerializzazioneException, IOException {
        ...
        ArrayList<PostiDisponibili> prenotateList = new ArrayList<PostiDisponibili>();

        ArrayList<PostiDisponibili> temp = new ArrayList<PostiDisponibili>();

        File file = new File(percorsoPrenotazioniAlberghi);
        ...
        if (file.length() == 0) {
            ...
            sobj.serializza(prenotateList, percorsoPrenotazioniAlberghi);
            return true;
        } else {

            DeserializzaOggetti dobj = new DeserializzaOggetti();
            prenotateList = (ArrayList<PostiDisponibili>) dobj.deserializza(percorsoPrenotazioniAlberghi);
            for (GregorianCalendar data_prenotazione : datePrenotazione) {
